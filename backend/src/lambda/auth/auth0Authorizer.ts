...
const jwksUrl = 'https://dev-um08jws6h2ouxw13.us.auth0.com/.well-known/jwks.json'
...
async function verifyToken(authHeader: string): Promise<JwtPayload> {
  ...
  const pem = transformCertToPem(signingKey.x5c[0]);
  return verify(token, pem, {
    algorithms: ['RS256']
  }) as JwtPayload
}

function transformCertToPem(cert) {
